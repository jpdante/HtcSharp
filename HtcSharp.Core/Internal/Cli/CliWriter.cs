...
namespace HtcSharp.Core.Internal.Cli {
    public class CliWriter : IWriter, IAsyncDisposable, IDisposable {
        ...
        public CliWriter(Stream stream, Encoding encoding, int bufferSize = -1) {
        ...
        public Task WriteAsync(ReadOnlyMemory<char> value, CancellationToken cancellationToken = default) => _streamWriter.WriteLineAsync(value, cancellationToken);
        public Task WriteAsync(StringBuilder value, CancellationToken cancellationToken = default)  => _streamWriter.WriteLineAsync(value, cancellationToken);
        public Task WriteAsync(string value) => _streamWriter.WriteLineAsync(value);
        public Task WriteAsync(char value)  => _streamWriter.WriteLineAsync(value);
        public Task WriteAsync(char[] buffer, int index, int count)  => _streamWriter.WriteLineAsync(buffer, index, count);
